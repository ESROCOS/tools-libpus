	...
	if(PUS_NO_ERROR == errorCode )
	{
		...
		else if( pus_TC_23_3_reportFileAttributes == subtype )
		{
			...
			if(PUS_NO_ERROR == errorCode)
			{
                ...
                apid = pusservices_getApid();//st23_RI_getApid(&apid);
                seqCount = pusservices_getTmSequenceCount();//st23_RI_getSequenceCount(&seqCount);
				...
				if( PUS_NO_ERROR == errorCode)
				{
					pusservices_RI_newTm(&tm);//st23_RI_newTm(&tm);
				}
			}
		}
