...
void st17_RI_ACK(const asn1SccPusPacket *IN_tcPacket, const asn1SccPusSubserviceType *IN_reportType, const asn1SccPusSt01FailureCode *IN_error, const asn1SccPusSt01FailureInfo *IN_errorInfo, const asn1SccPusStepId *IN_step)
{
#ifdef __unix__
...
#ifdef __unix__
        printf ("** Encoding error in st17_RI_ACK!!\n");
#endif
    ...
    extern void vm_st17_ACK(void *, size_t, void *, size_t, void *, size_t, void *, size_t, void *, size_t);

    vm_st17_ACK(IN_buf_tcPacket, size_IN_buf_tcPacket, IN_buf_reportType, size_IN_buf_reportType, IN_buf_error, size_IN_buf_error, IN_buf_errorInfo, size_IN_buf_errorInfo, IN_buf_step, size_IN_buf_step);

}

void st17_RI_getApid(asn1SccPusApid *OUT_apid)
{
#ifdef __unix__
    static int innerMsc = -1;
    if (-1 == innerMsc)
    ...
    if (1 == innerMsc) {
        ...
        printf ("\nINNER: st17,onboardapid,getApid,%lld\n", msc_time);
        fflush(stdout);
    }
#endif
    ...
    /* Call to VM callback function */
    extern void vm_st17_getApid(void *, size_t *);

    vm_st17_getApid(OUT_buf_apid, &size_OUT_buf_apid);
...
void st17_RI_getSequenceCount(asn1SccPusSequenceCount *OUT_sequenceCount)
{
#ifdef __unix__
    static int innerMsc = -1;
    if (-1 == innerMsc)
    ...
    if (1 == innerMsc) {
        ...
        printf ("\nINNER: st17,onboardapid,getSequenceCount,%lld\n", msc_time);
        fflush(stdout);
    }
#endif
    ...
    /* Call to VM callback function */
    extern void vm_st17_getSequenceCount(void *, size_t *);

    vm_st17_getSequenceCount(OUT_buf_sequenceCount, &size_OUT_buf_sequenceCount);
...
void st17_RI_newTm(const asn1SccPusPacket *IN_tmPacket)
{
#ifdef __unix__
    static int innerMsc = -1;
    if (-1 == innerMsc)
...
#ifdef __unix__
        printf ("** Encoding error in st17_RI_newTm!!\n");
#endif
        /* Crash the application due to message loss */
        extern void abort (void);
        abort();
    ...
    /* Call to VM callback function */
    extern void vm_st17_newTm(void *, size_t);

    vm_st17_newTm(IN_buf_tmPacket, size_IN_buf_tmPacket);
