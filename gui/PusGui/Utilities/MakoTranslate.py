...
class MakoTranslate(object):
    def __init__(self):
        ...
        read_value = " "
        while read_value is not None:
            read_value = pb.pus_st08_getFunctionName(param_id)
            if read_value is not None:
                self.st08_params[read_value] = param_id
            param_id += 1
        ...
        self.values = [self.version, self.st3_12_params, self.st5_events, self.st08_params, self.st20_params]
