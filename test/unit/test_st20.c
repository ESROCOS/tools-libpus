...
void test_st20()
{
	...
	int32_t valInt;
	CU_ASSERT_EQUAL(PUS_NO_ERROR, pus_parameters_setONBOARD_PARAM_INT01(-23));
	CU_ASSERT_EQUAL(PUS_NO_ERROR, pus_parameters_getONBOARD_PARAM_INT01(&valInt));
	CU_ASSERT_EQUAL(-23, valInt);

	double valDou;
	CU_ASSERT_EQUAL(PUS_NO_ERROR, pus_parameters_setONBOARD_PARAM_REAL01(-23.23));
	CU_ASSERT_EQUAL(PUS_NO_ERROR, pus_parameters_getONBOARD_PARAM_REAL01(&valDou));
	CU_ASSERT_EQUAL(-23.23, valDou);

	uint32_t valUint;
	CU_ASSERT_EQUAL(PUS_NO_ERROR, pus_parameters_setONBOARD_PARAM_UINT01(23));
	CU_ASSERT_EQUAL(PUS_NO_ERROR, pus_parameters_getONBOARD_PARAM_UINT01(&valUint));
	CU_ASSERT_EQUAL(23, valUint);

	uint8_t valByte;
	CU_ASSERT_EQUAL(PUS_NO_ERROR, pus_parameters_setONBOARD_PARAM_BYTE01(0x23));
	CU_ASSERT_EQUAL(PUS_NO_ERROR, pus_parameters_getONBOARD_PARAM_BYTE01(&valByte));
	CU_ASSERT_EQUAL(0x23, valByte);

	bool valBool;
	CU_ASSERT_EQUAL(PUS_NO_ERROR, pus_parameters_setONBOARD_PARAM_BOOL01(true));
	CU_ASSERT_EQUAL(PUS_NO_ERROR, pus_parameters_getONBOARD_PARAM_BOOL01(&valBool));
