...
'''Functions to process the ST[18] OBCP management.'''
...
def generate_st18_config(jsonDir, outDir):
    '''Generate the C files for the PUS ST[18] service configuration'''
    # File paths
    scriptDir = os.path.dirname(os.path.realpath(__file__))
    schemaFile = os.path.join(scriptDir, 'schemas', 'st18_schema.json')
    headerTemplate = os.path.join(scriptDir, 'templates', 'pus_st18_config.h.mako')
    srcTemplate = os.path.join(scriptDir, 'templates', 'pus_st18_config.c.mako')
    missionFile = os.path.join(jsonDir, 'st18_config.json')

    outHeaderFile = os.path.join(outDir, 'include', 'pus_st18_config.h')
    outSrcFile = os.path.join(outDir, 'src', 'pus_st18_config.c')
    
    if not os.path.isfile(missionFile):
        raise Exception('ST[18] configuration file not found at {}'.format(missionFile))

    # Load and validate configuration
    print(' - generate configuration for service ST[18]')
    schema = loadJson(schemaFile)
    configData = loadJson(missionFile)
    try:
        jsonschema.validate(configData, schema)
    except Exception as err:
        perror('Error in ST[18] service configuration {}:\n{}'.format(missionFile, err))
        raise
    ...
    try:
        ...
        perror('Error writing ST[18] generated code to {}:\n{}'.format(outHeaderFile, err))
        raise
